  ...
  <div class="space-y-6">
    ...
    <template v-if="guest">
      ...
      <TabView v-model:activeIndex="activeTabIndex">
        ...
        <!-- SCHEDULE TAB -->
        <TabPanel :header="'Schedule (' + tabData.schedule.length + ')'" value="1">
          <div class="flex justify-end mb-4">
            <Button
          ...
          <Card v-if="showAddSchedule" class="mb-4">
            <template #content>
              ...
              <Button label="Create Event" icon="pi pi-check" size="small" class="mt-3" :loading="saving" @click="addScheduleEvent" />
            </template>
          </Card>
          <DataTable :value="tabData.schedule" stripedRows>
        ...
        <!-- TRAVEL TAB -->
        <TabPanel :header="'Travel (' + tabData.travel.length + ')'" value="2">
          <div class="flex justify-end mb-4">
            <Button
          ...
          <Card v-if="showAddTravel" class="mb-4">
            <template #content>
              ...
              <Button label="Add Travel" icon="pi pi-check" size="small" class="mt-3" :loading="saving" @click="addTravel" />
            </template>
          </Card>
          <DataTable :value="tabData.travel" stripedRows>
        ...
        <!-- ACCOMMODATIONS TAB -->
        <TabPanel :header="'Accom (' + tabData.accommodations.length + ')'" value="3">
          <DataTable :value="tabData.accommodations" stripedRows>
        ...
        <!-- DIETARY TAB -->
        <TabPanel :header="'Dietary (' + tabData.dietary.length + ')'" value="4">
          <DataTable :value="tabData.dietary" stripedRows>
        ...
        <!-- AUTOGRAPHS TAB -->
        <TabPanel :header="'Autographs (' + tabData.autographs.length + ')'" value="5">
          <DataTable :value="tabData.autographs" stripedRows>
...
function addScheduleEvent(): void {
  if (!guest.value) return
  ...
  savingCount.value++
  const eventData = { ...newEvent, 'Guest ID': String(guest.value['Guest ID']) }
  api.call<WriteResult>('createScheduleEvent', { data: eventData })
    .then((result) => {
...
function addTravel(): void {
  if (!guest.value) return
  if (!newTravel['Travel Type']) {
    toast.add({ severity: 'warn', summary: 'Validation', detail: 'Travel Type is required', life: 3000 })
    return
  }
  savingCount.value++
  const travelData = { ...newTravel, 'Guest ID': String(guest.value['Guest ID']) }
  api.call<WriteResult>('createTravel', { data: travelData })
    .then((result) => {
      if (result.status === 'APPLIED') {
